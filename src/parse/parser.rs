    ...
    fn parse_node(&mut self) -> Option<Spanned<SyntaxNode>> {
        ...
        Some(match token.v {
            ...
            Token::Raw { backticks, lang, raw, terminated } => {
                ...
                let (lines, block) = if backticks > 1 {
                    trim_and_split_raw(raw)
                } else {
                    (split_lines(raw), false)
                };

                self.with_span(SyntaxNode::Raw(Raw { lang, lines, block }))
            }
...
impl Parser<'_> {
    ...
    fn parse_value(&mut self) -> Option<Spanned<Expr>> {
        ...
        Some(match token {
            ...
            // This could be a dictionary or a parenthesized expression. We parse as
            // a dictionary in any case and coerce the dictionary into a value if that's
            // possible (length 1 and no trailing comma).
            Token::LeftParen => {
                self.start_group(Group::Paren);
